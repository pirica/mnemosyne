#!/usr/bin/env python3
"""Split batch-4 knowledge graph into 2 parts."""
import json, math, os
...
for part_idx, file_list in enumerate(part_files, 1):
    # Build set of file paths in this part
    part_file_set = set(file_list)
    
    # Collect all node IDs whose filePath is in this part (file nodes + sub-file nodes)
    part_node_ids = set()
    part_nodes = []
    seen_ids = set()
    ...
    for node in all_nodes:
        ...
        # File nodes match exactly; sub-file nodes match by filePath
        if fpath in part_file_set or node["id"].startswith(f"file:{fpath}"):
            if node["id"] not in seen_ids:
                part_node_ids.add(node["id"])
                part_nodes.append(node)
                seen_ids.add(node["id"])
    
    # Also add file nodes for every file in this part
    for fp in file_list:
        fid = f"file:{fp}"
        if fid not in seen_ids:
            # Find the file node
            for node in all_nodes:
                if node["id"] == fid and fid not in seen_ids:
                    part_nodes.append(node)
                    part_node_ids.add(fid)
                    seen_ids.add(fid)
                    break
    
    # Collect edges where source is in our nodes
    part_edges = []
    edge_seen = set()
    for edge in all_edges:
        key = (edge["source"], edge["target"], edge["type"])
        if edge["source"] in part_node_ids and key not in edge_seen:
    ...
    print(f"  Node IDs: {[n['id'] for n in part_nodes[:3]]}...")
